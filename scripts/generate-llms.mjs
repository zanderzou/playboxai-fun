...
const localized=locales.flatMap(([language,slug])=>[
  ...
  ...articles.map(a=>`- [${language}: ${label(localizedComparisons[slug][a.slug.replace('playbox-ai-vs-','')].title)}](${origin}/${slug}/blog/${a.slug}/)`),
  ...optional.map(([,page])=>`- [${language}: ${label(infoCopy[slug][page].title)}](${origin}/${slug}/${page}/)`),
]);
...
if(process.argv.includes('--check')){
 ...
 console.log(`${new URL(origin).hostname}: llms.txt current, ${articles.length*(locales.length+1)} article links verified`);
